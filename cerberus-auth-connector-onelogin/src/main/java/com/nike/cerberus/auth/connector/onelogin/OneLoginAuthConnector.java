...
public class OneLoginAuthConnector implements AuthConnector {
   ...
   * @return A set of Strings consisting of the ldap groups that were parsed from the provided
   *     string
   */
  protected Set<String> parseLdapGroups(final String ldapGroups) {
    ...
    String escapedLdapGroups =
        StringEscapeUtils.unescapeXml(StringEscapeUtils.unescapeXml(ldapGroups));
   ...
   */
  protected SessionLoginTokenData verifyFactor(
      final String deviceId, final String stateToken, final String otpToken) {

    final VerifyFactorResponse verifyFactorResponse =
        oneLoginClient.verifyFactor(deviceId, stateToken, otpToken);

    if (verifyFactorResponse.getStatus().isError()) {
      String msg =
          String.format(
              "stateToken: %s failed to verify 2nd factor for reason: %s",
  ...
  /**
   * Request for creating a session login token. This is used to validate a user's credentials with
   * OneLogin.
   *
   ...
   * @return Session login token
   */
  protected SessionLoginTokenData createSessionLoginToken(
      final String username, final String password) {

    final CreateSessionLoginTokenResponse createSessionLoginTokenResponse =
        oneLoginClient.createSessionLoginToken(username, password);
    ...
    if (createSessionLoginTokenResponse.getStatus().isError()) {
      String msg =
          String.format(
              "The user %s failed to authenticate for reason: %s",
              username, createSessionLoginTokenResponse.getStatus().getMessage());
      if (statusCode == 400
          && StringUtils.startsWithIgnoreCase(
              createSessionLoginTokenResponse.getStatus().getMessage(), "MFA")) {
        throw ApiException.newBuilder()
