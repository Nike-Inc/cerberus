...
public class AuthenticationServiceV1 {
    ...
    private final KmsServiceV1 kmsService;
    ...
    @Inject
    public AuthenticationServiceV1(final SafeDepositBoxDao safeDepositBoxDao,
                                   final AwsIamRoleDao awsIamRoleDao,
                                   final AuthConnector authConnector,
                                   final KmsServiceV1 kmsService,
    ...
    public IamRoleAuthResponse authenticate(IamRoleCredentialsV1 credentials) {
        ...
        meta.put(VaultAuthPrincipalV1.METADATA_KEY_AWS_ACCOUNT_ID, credentials.getAccountId());
        meta.put(VaultAuthPrincipalV1.METADATA_KEY_AWS_IAM_ROLE_NAME, credentials.getRoleName());
        meta.put(VaultAuthPrincipalV1.METADATA_KEY_AWS_REGION, credentials.getRegion());
        meta.put(VaultAuthPrincipalV1.METADATA_KEY_USERNAME, arn);

        // We will allow specific ARNs access to the user portions of the API
        if (getAdminRoleArnSet().contains(arn)) {
            meta.put(VaultAuthPrincipalV1.METADATA_KEY_IS_ADMIN, Boolean.toString(true));
        }
     ...
     */
    public AuthResponse refreshUserToken(final VaultAuthPrincipalV1 authPrincipal) {
    ...
    private VaultAuthResponse generateToken(final String username, final Set<String> userGroups) {
        ...
        meta.put(VaultAuthPrincipalV1.METADATA_KEY_IS_ADMIN, String.valueOf(isAdmin));
        meta.put(VaultAuthPrincipalV1.METADATA_KEY_GROUPS, StringUtils.join(userGroups, ','));
    ...
    private Set<String> buildPolicySet(final String accountId, final String iamRoleName) {
        ...
        final List<SafeDepositBoxRoleRecord> sdbRoles =
                safeDepositBoxDao.getIamRoleAssociatedSafeDepositBoxRoles(
                        String.format(AwsIamRoleArnParser.AWS_IAM_ROLE_ARN_TEMPLATE, accountId, iamRoleName));
     ...
     * @return KMS Key id
     */
    private String getKeyId(IamRoleCredentialsV1 credentials) {
        final Optional<AwsIamRoleRecord> iamRole =
                awsIamRoleDao.getIamRole(String.format(AwsIamRoleArnParser.AWS_IAM_ROLE_ARN_TEMPLATE,
