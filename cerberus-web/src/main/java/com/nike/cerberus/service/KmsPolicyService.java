...
public class KmsPolicyService {
  ...
  /**
   * Validates that the given key policy grants appropriate permissions to CMS and allows the given
   * IAM principal to access the KMS key.
   *
   * @param policyJson - The KMS key policy as a String
   * @return true if the policy is valid, false if the policy contains an ID because the ARN had
   ...
   *
   * <p>This is important because when an IAM principal is deleted and recreated with the same name,
   * then the recreated principal cannot access the KMS key until the key policy is regenerated --
   * updating the policy permissions to allow the ARN of the recreated principal instead of the ID
   * of the deleted principal.
   *
   * @param policyJson - The KMS key policy as a String
   */
  ...
  /**
   * Validate that the IAM principal for the CMS has permissions to schedule and cancel deletion of
   * the KMS key.
   *
   * @param policyJson - The KMS key policy as a String
   */
  protected boolean cmsHasKeyDeletePermissions(String policyJson) {
    try {
      ...
      logger.error(
          "Failed to validate that CMS can delete KMS key, there may be something wrong with the policy",
          e);
    }
  ...
  /**
   * Overwrite the policy statement for CMS with the standard statement. Add the standard statement
   * for CMS to the policy if it did not already exist.
   *
   ...
   *
   * <p>This is important when updating the KMS policy because if the IAM principal has been deleted
   * then the KMS policy will contain the principal 'ID' instead of the ARN, which renders the
   * policy invalid when calling {@link
   * com.amazonaws.services.kms.AWSKMSClient#putKeyPolicy(PutKeyPolicyRequest)}.
   *
  ...
  protected boolean statementAppliesToPrincipal(Statement statement, String principalArn) {
    ...
    return statement.getPrincipals().stream()
        .anyMatch(principal -> StringUtils.equals(principal.getId(), principalArn));
  }
  ...
  public String generateStandardKmsPolicy(String iamRoleArn) {
    ...
    iamRoleUsageStatement.withPrincipals(new Principal(AWS_PROVIDER, iamRoleArn, false));
