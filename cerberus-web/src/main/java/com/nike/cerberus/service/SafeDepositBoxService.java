...
public class SafeDepositBoxService {
   ...
   */
  public List<SafeDepositBoxSummary> getAssociatedSafeDepositBoxes(
      final CerberusPrincipal principal) {
    ...
    switch (principal.getPrincipalType()) {
      case IAM:
        ...
        if (awsIamRoleArnParser.isAssumedRoleArn(principalName)) {
          ...
          sdbRecords =
              safeDepositBoxDao.getAssumedRoleAssociatedSafeDepositBoxes(
                  principalName, iamRoleArn, rootArn);
        } else {
          sdbRecords =
              safeDepositBoxDao.getIamPrincipalAssociatedSafeDepositBoxes(principalName, rootArn);
        }
        break;
      case USER:
        sdbRecords =
  ...
  /**
   * Queries the data store for the specific safe deposit box by ID.
   *
   * @param id The unique identifier for the safe deposit box to lookup
   * @return The safe deposit box, if found
   */
  public SafeDepositBoxV1 getSDBAndValidatePrincipalAssociationV1(String id) {
    return convertSafeDepositBoxV2ToV1(getSDBAndValidatePrincipalAssociationV2(id));
  }
   ...
   * @throws ApiException Throws an exception if the requesting principal has no permissions
   *     associated with the requested SDB.
   * @throws ApiException Throws an exception if the SDB Id is invalid
   */
  public SafeDepositBoxV2 getSDBAndValidatePrincipalAssociationV2(String sdbId) {
  ...
  protected SafeDepositBoxV2 getSDBFromRecordV2(SafeDepositBoxRecord safeDepositBoxRecord) {
    ...
    if (possibleOwner.isEmpty()) {
      log.error("Detected Safe Deposit Box without owner! ID={}", id);
    } else {
  ...
  public SafeDepositBoxV2 createSafeDepositBoxV2(
    ...
    userGroupPermissionService.grantUserGroupPermissions(
        boxRecordToStore.getId(), userGroupPermissionSet, user, now);

    iamPrincipalPermissionService.grantIamPrincipalPermissions(
        boxRecordToStore.getId(), iamRolePermissionSet, user, now);
   ...
   */
  @Transactional
  public void updateSafeDepositBoxV1(
   ...
   */
  @Transactional
  public SafeDepositBoxV2 updateSafeDepositBoxV2(
    ...
    final SafeDepositBoxRecord boxToUpdate =
        buildBoxToUpdate(id, safeDepositBox, principalName, now);
    final Set<UserGroupPermission> userGroupPermissionSet =
        safeDepositBox.getUserGroupPermissions();
    final Set<IamPrincipalPermission> iamRolePermissionSet =
        safeDepositBox.getIamPrincipalPermissions();
   ...
   */
  @Transactional
  public void deleteSafeDepositBox(final String id) {
    final SafeDepositBoxV2 box = getSDBAndValidatePrincipalAssociationV2(id);
  ...
  private Optional<String> extractOwner(Set<UserGroupPermission> userGroupPermissions) {
    ...
    final Optional<UserGroupPermission> ownerPermission =
        userGroupPermissions.stream()
            .filter(perm -> StringUtils.equals(perm.getRoleId(), ownerRole.get().getId()))
            .findFirst();
   ...
   */
  private SafeDepositBoxRecord buildBoxToUpdate(
      final String id,
      final SafeDepositBoxV2 safeDepositBox,
      final String user,
  ...
  /**
   * Sorts out the set of permissions into, grant, update and revoke sets. After that it applies
   * those changes.
   */
  protected void modifyUserGroupPermissions(
      final SafeDepositBoxV2 currentBox,
      final Set<UserGroupPermission> userGroupPermissionSet,
      final String user,
      final OffsetDateTime dateTime) {
    ...
    toDeleteSet.addAll(
        currentBox.getUserGroupPermissions().stream()
    ...
    userGroupPermissionService.grantUserGroupPermissions(
        safeDepositBoxId, toAddSet, user, dateTime);
    userGroupPermissionService.updateUserGroupPermissions(
        safeDepositBoxId, toUpdateSet, user, dateTime);
  ...
  protected void modifyIamPrincipalPermissions(
    ...
    toDeleteSet.addAll(
        currentBox.getIamPrincipalPermissions().stream()
  ...
  public void restoreSafeDepositBox(SafeDepositBoxV2 safeDepositBox, String adminUser) {
    ...
    Optional<SafeDepositBoxRecord> existingBoxRecord =
        safeDepositBoxDao.getSafeDepositBox(safeDepositBox.getId());
    if (existingBoxRecord.isPresent()) {
      ...
      modifyUserGroupPermissions(
          existingBox, safeDepositBox.getUserGroupPermissions(), adminUser, now);
      modifyIamPrincipalPermissions(
          existingBox, safeDepositBox.getIamPrincipalPermissions(), adminUser, now);
    } else {
      ...
      userGroupPermissionService.grantUserGroupPermissions(
          safeDepositBox.getId(), safeDepositBox.getUserGroupPermissions(), adminUser, now);

      iamPrincipalPermissionService.grantIamPrincipalPermissions(
          safeDepositBox.getId(), safeDepositBox.getIamPrincipalPermissions(), adminUser, now);
    }
  }
   ...
   */
  @Transactional
  public void overrideOwner(String sdbName, String newOwner, String adminUser) {
    String sdbId =
        getSafeDepositBoxIdByName(sdbName)
            .orElseThrow(
                () ->
  ...
  // TODO rename, perms not dealt with here any more
  public SafeDepositBoxV2 getSafeDepositBoxDangerouslyWithoutPermissionValidation(String sdbId) {
    Optional<SafeDepositBoxRecord> safeDepositBoxRecordOptional =
        safeDepositBoxDao.getSafeDepositBox(sdbId);

    return getSDBFromRecordV2(
