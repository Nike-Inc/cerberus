...
public class SecureDataService {
  ...
  public void writeSecret(String sdbId, String path, String plainTextPayload, String principal) {
    ...
    // Make sure to encrypt payload as a String, then convert to bytes to mimic the previous
    // encryption flow
    ...
    // Fetch the current version if there is one, so that on update it can be moved to the versions
    // table
    Optional<SecureDataRecord> secureDataRecordOpt =
  ...
  @Transactional
  public void writeSecureFile(
      String sdbId, String path, byte[] bytes, int sizeInBytes, String principal) {
  ...
  /**
   * Attempts to deserialize the plain text payload and determine how many key value pairs it
   * contains, in order to capture this metadata metric for KPI reporting.
   *
  ...
  protected int getTopLevelKVPairCount(String plainTextPayload) {
    ...
    try {
      Map<String, Object> data =
          objectMapper.readValue(plainTextPayload, new TypeReference<HashMap<String, Object>>() {});
      kvCount = data.size();
    } catch (Exception e) {
  ...
  public Optional<SecureData> readSecret(String sdbId, String path) {
    ...
    Optional<SecureDataRecord> secureDataRecordOpt =
        secureDataDao.readSecureDataByPathAndType(sdbId, path, SecureDataType.OBJECT);
    if (!secureDataRecordOpt.isPresent()) {
  ...
  public Optional<SecureFileCurrent> readFile(String sdbId, String path) {
  ...
  public void restoreSdbSecrets(
      String sdbId, Map<String, Map<String, Object>> data, String principal) {
    data.forEach(
        (String path, Map<String, Object> secretsData) -> {
  ...
  /**
   * Method to list keys in the virtual tree structure This method is designed to mimic the Vault
   * ?list=true functionality to maintain the existing API contract.
   *
   * <p>ex: given the following tree structure app/foo/bar/bam app/foo/bam app/bam/foo
   *
   * <p>if you call listKeys with partialPath = "app/foo" or "app/foo/" you will receive the
   * following set of keys ["bar/", "bam"]
   *
  ...
  public Set<String> listKeys(String sdbId, String partialPath) {
    ...
    String[] pArray =
        secureDataDao.getPathsByPartialPathAndType(sdbId, partialPath, SecureDataType.OBJECT);
    ...
    for (int i = 0; i < pArray.length; i++) {
      ...
      keys.add(StringUtils.removeStart(fullPath, partialPath).replaceAll("\\/.*$", "/"));
    }
    return keys;
  }

  /**
   * Method to list file metadata in the virtual tree structure This method is designed to mimic the
   * Vault ?list=true functionality to maintain the existing API contract.
   *
   * <p>ex: given the following tree structure app/foo/bar/bam app/foo/bam app/bam/foo
   *
   * <p>if you call listSecureFilesSummaries with partialPath = "app/foo" or "app/foo/" you will
   * receive files matching the following set of keys ["bar/", "bam"]
   *
   * @param partialPath path to a node in the data structure that potentially has children
   * @return Array of keys if the key is a data node it will not end with "/"
   */
  public SecureFileSummaryResult listSecureFilesSummaries(
      String sdbId, String partialPath, int limit, int offset) {
    if (!partialPath.endsWith("/")) {
      partialPath = partialPath + "/";
    }
    ...
    List<SecureDataRecord> secureDataRecords =
        secureDataDao.listSecureDataByPartialPathAndType(
            sdbId, partialPath, SecureDataType.FILE, limit, offset);
   ...
   *
   * <p>ex: assume the following paths have secure data app/test/foo/1 app/test/foo/2 app/test/foo/3
   * app/test/foo/4/bar app/test/foo/4/bam app/test/bam and you call this method with path =
   * app/test/foo/ all the above secrets except app/test/bam will be deleted
   *
  ...
  public void deleteSecret(String sdbId, String path, SecureDataType type, String principal) {
    ...
    SecureDataRecord secureDataRecord =
        secureDataDao
            .readSecureDataByPathAndType(sdbId, path, type)
            .orElseThrow(() -> new ApiException(DefaultApiError.ENTITY_NOT_FOUND));
  ...
  boolean secureDataHasBeenUpdated(SecureDataRecord secureDataRecord) {
    ...
    boolean createdTsSameAsUpdatedTs =
  ...
  /**
   * Rotate data keys by re-encrypting data. The oldest data keys in both secure data and secure
   * data version that are expired (based on rotation interval) will be considered for key rotation.
   *
   ...
   * @param rotationIntervalInDays Data keys generated older than X days will be considered for key
   *     rotation
   */
  public void rotateDataKeys(int numberOfKeys, int pauseTimeInMillis, int rotationIntervalInDays) {
    ...
    List<DataKeyInfo> oldestDataKeyInfos =
        secureDataDao.getOldestDataKeyInfo(expiredTs, numberOfKeys);
    ...
    log.info(
        "Re-encrypted {} secure data and {} secure data version entries.", counter[0], counter[1]);
  }
  ...
  protected void reencryptData(String id) {
    ...
    byte[] reencryptedBytes =
        reencrypt(secureDataRecord.getType(), ciphertextBytes, secureDataRecord.getPath());
    OffsetDateTime now = dateTimeSupplier.get();
  ...
  protected void reencryptDataVersion(String versionId) {
    ...
    Optional<SecureDataVersionRecord> secureDataVersionRecord =
        secureDataVersionDao.readSecureDataVersionByIdLocking(versionId);
