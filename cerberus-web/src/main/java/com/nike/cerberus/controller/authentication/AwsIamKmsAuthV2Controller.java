...
public class AwsIamKmsAuthV2Controller {
  ...
  @Autowired
  public AwsIamKmsAuthV2Controller(AuthenticationService authenticationService,
  ...
  public EncryptedAuthDataWrapper authenticate(@RequestBody AwsIamKmsAuthRequest request) {
    ...
    try {
      ...
      eventProcessorService.ingestEvent(createBaseAuditableEvent(request.getIamPrincipalArn())
        .withAction(String.format("Failed to authenticate in region %s, for reason: %s",
          request.getRegion(),
          e.getApiErrors().stream().map(ApiError::getMessage).collect(Collectors.joining(","))))
        .withSuccess(false)
        .build()
      );
      throw e;
    ...
    eventProcessorService.ingestEvent(createBaseAuditableEvent(request.getIamPrincipalArn())
      .withAction(String.format("Successfully authenticated in region %s", request.getRegion()))
      .build()
    );
